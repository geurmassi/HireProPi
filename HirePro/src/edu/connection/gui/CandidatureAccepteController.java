...
import edu.connection.api.EmailSenderAla;
import edu.connection.api.PDFReader;
import edu.connection.api.PDFViewer;
import edu.connection.entities.Condidature;
import edu.connection.entities.EtatCandidature;
import edu.connection.services.CondidatureCRUD;
 ...
 * @author LENOVO
 */
public class CandidatureAccepteController implements Initializable {
    ...
    @FXML
    private ComboBox<EtatCandidature> comboEtat;
    @FXML
        ...
        if (!labelValueUser.isEmpty()) {
            ...
            if (!numericValueUser.isEmpty()) {
                ...
                comboBoxOffre.setOnAction(event -> {
                    ...
                    ObservableList<EtatCandidature> etatList = FXCollections.observableArrayList(EtatCandidature.values());
                    etatList.remove(EtatCandidature.en_attente); // Remove the "en_attente" value from the list
                    comboEtat.setItems(etatList);
                    // Perform the necessary logic to fetch the candidatures based on the selected poste
                    ObservableList<Condidature> liste = FXCollections.observableList(cm.getCandidaturesByPosteAcceptee(selectedPoste));
    ...
    @FXML
    private void condidatureEffacer(ActionEvent event) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("CandidatureRefusee.fxml"));
            Parent root = loader.load();
            lbPoste.getScene().setRoot(root);
            CandidatureRefuseeController controller = loader.getController();
    ...
    private void updateEtat(ActionEvent event) {
        ...
        EtatCandidature selectedEtat = comboEtat.getValue();

        if (selectedCondidature != null && selectedEtat != null) {
            // Compare the selectedEtat with the previous etat
            EtatCandidature previousEtat = selectedCondidature.getEtat();
            if (!selectedEtat.equals(previousEtat)) {
                ...
                EmailSenderAla emailSender = new EmailSenderAla();
                if (selectedEtat == EtatCandidature.refusé) {
                    String objet = "Résultat de votre candidature - " + selectedPoste;
                    String messagerefuse = "Cher(e) " + name + " ,\n"
                            + "\n"
                            + "Nous vous remercions pour l'intérêt que vous avez porté à notre entreprise et pour avoir soumis votre candidature pour "
                            + selectedPosteMessage + " . Après avoir soigneusement examiné votre profil, nous regrettons de vous informer que votre candidature n'a pas "
                            + "été retenue pour le poste.\n"
                            + "\n"
                            + "Nous tenons à souligner que la sélection des candidats est un processus compétitif et que notre décision est basée sur "
                            + "plusieurs critères spécifiques aux besoins de l'entreprise et aux compétences requises pour le poste.\n"
                            + "\n"
                            + "Nous apprécions sincèrement le temps et les efforts que vous avez consacrés à postuler chez nous. Votre profil "
                            + "professionnel est intéressant et nous vous encourageons à continuer à rechercher de nouvelles opportunités qui "
                            + "correspondent à vos compétences et à vos aspirations.\n"
                            + "\n"
                            + "Nous vous souhaitons beaucoup de succès dans vos recherches futures et nous vous remercions encore une fois pour l'intérêt"
                            + " que vous avez manifesté envers notre entreprise.\n"
                            + "\n"
                            + "Cordialement,\n"
                            + "\n"
                            + "Mensieur " + nameRecruteur;
                    emailSender.sendEmail(mc.retrieveUserEmailByIdUser(selectedCondidature.getIdUtilisateur()), objet, messagerefuse);
                    System.out.println("mail refuser");
                }
                ...
                alert.showAndWait();
                comboEtat.setValue(EtatCandidature.en_attente);
                comboEtat.setVisible(false);
    ...
    @FXML
    private void candidatureAttente(ActionEvent event) {
        try {
